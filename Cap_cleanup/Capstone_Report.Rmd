...
My mentor suggested that I could treat the problem as a classification problem, whether deaths occurred or not. So I divided the data frame into two groups, one with deaths and one without fatality, and saved a new variable. Then I used this newly created variable and ran the random forest algorithm on my data again. The out-of-bag error is 32.85%. We can take a look at how well our model is by plotting the AUC (Area Under the Curve). Here we see that AUC = 0.72, which means there is 72% chance that our model will rank a randomly chosen postive example (deaths exist) higher than a randomly chosen negative example (no deaths).    
...
I also plotted the accuracy plot to show how the accuracy changes as a function of cutoff value. And we can see when the cut off value is ~ 0.5, we achieve the highest accuracy.  
...
We can also show how f-score changes as a function of cutoff, and the f score for this model. 
```{r, message = FALSE,warning = FALSE, echo = FALSE, fig.height = 5, fig.align = "center"}
